...
export async function judgeTrajectory(
  ...
  if (!hasApiKey || !ModelRouter) {
    console.warn('[WARN] No API key or ModelRouter unavailable, using heuristic judgment');
...
function heuristicJudge(trajectory: Trajectory, query: string): Verdict {
  ...
  for (const step of steps) {
    // Check exitCode (numeric)
    if (step.exitCode === 0) {
      successSignals++;
    } else if (step.exitCode != null && step.exitCode !== 0) {
      errorCount++;
    }

    // Check string-based indicators
  ...
  // Success if (positive signals AND no errors) OR (more successes than errors)
  const isSuccess = steps.length > 0 &&
    ((successSignals > 0 && errorCount === 0) || (successSignals > errorCount));
