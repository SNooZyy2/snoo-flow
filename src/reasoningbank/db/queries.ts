...
export function fetchMemoryCandidates(options: {
  ...
  query += ` ORDER BY p.confidence DESC, p.usage_count DESC LIMIT 1000`;
  ...
  return rows.filter((row: any) => {
    ...
    return buf.length % 4 === 0; // skip corrupted embeddings
  }).map((row: any) => {
...
export function storeTrajectory(trajectory: Omit<TaskTrajectory, 'created_at'>): void {
  ...
  db.prepare(`
    ...
    (task_id, agent_id, query, trajectory_json, started_at, ended_at,
     judge_label, judge_conf, judge_reasons, matts_run_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    trajectory.task_id,
    trajectory.agent_id,
